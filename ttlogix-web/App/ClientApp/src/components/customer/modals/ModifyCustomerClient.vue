<script lang="ts" setup>
    ...
    const emit = defineEmits<{ (e: 'close'): void, (e: 'proceed', currentCompany: CustomerClient, addNext: boolean): void }>();
    ...
    const currentCustomerClient = ref(new CustomerClient());
    ...
    const loadCustomer = async () => {
        ...
        if (props.modalBase.customProps) {
            currentCustomerClient.value = await store.dispatch(ActionTypes.CS_GET_CUSTOMER_CLIENT, { customerClientCode: props.modalBase.customProps });
            setAddressBooks(currentCustomerClient.value.companyCode);
            setAddressContacts(currentCustomerClient.value.primaryAddress);
            setPrimaryAddress(currentCustomerClient.value.primaryAddress);
            setBillingAddress(currentCustomerClient.value.billingAddress);
            setShippingAddress(currentCustomerClient.value.shippingAddress);
            setPic(currentCustomerClient.value.pic1);
            setPic(currentCustomerClient.value.pic2, false);
        }
        loading.value = false;
    }
    ...
    const setPrimaryAddress = (abCode: string | null = null) => {
        if (abCode) primaryAddress.value = (addressBooks.value.filter((ab) => ab.code == (abCode ? abCode : currentCustomerClient.value.primaryAddress)))[0];
        else primaryAddress.value = new CompanyProfileAddressBook();
    }
    const setBillingAddress = (abCode: string | null = null) => {
        if (abCode) billingAddress.value = (addressBooks.value.filter((ab) => ab.code == (abCode ? abCode : currentCustomerClient.value.billingAddress)))[0];
        else billingAddress.value = new CompanyProfileAddressBook();
    }
    const setShippingAddress = (abCode: string | null = null) => {
        if (abCode) shippingAddress.value = (addressBooks.value.filter((ab) => ab.code == (abCode ? abCode : currentCustomerClient.value.shippingAddress)))[0];
    ...
    const setPic = (selectedContactCode: string | null = null, isPic1: boolean = true) => {
        if (selectedContactCode) {
            if (isPic1) pic1.value = (addressContacts.value.filter((ac) => ac.code == (selectedContactCode ? selectedContactCode : currentCustomerClient.value.pic1)))[0];
            else pic2.value = (addressContacts.value.filter((ac) => ac.code == (selectedContactCode ? selectedContactCode : currentCustomerClient.value.pic2)))[0];
        }
        else {
    ...
    const primaryAddressChange = () => {
        setPrimaryAddress(currentCustomerClient.value.primaryAddress);
        setAddressContacts(currentCustomerClient.value.primaryAddress);
        currentCustomerClient.value.pic1 = null;
        currentCustomerClient.value.pic2 = null;
    ...
    const companyChange = () => {
        ...
        currentCustomerClient.value.pic1 = null;
        currentCustomerClient.value.pic2 = null;
        currentCustomerClient.value.primaryAddress = null;
        currentCustomerClient.value.shippingAddress = null;
        currentCustomerClient.value.billingAddress = null;
    ...
    const canSave = () => {
        let cst = currentCustomerClient.value;
        return cst.code && cst.companyCode && cst.name && cst.primaryAddress && cst.billingAddress && cst.shippingAddress && cst.pic1 && cst.pic2;
    }
    ...
    const addNewAddressBook = () => {
        addAddressBookModal.value.customProps = currentCustomerClient.value.companyCode;
    ...
    const addNewAddressContact = () => {
        ...
        addAddressContactModal.value.customProps = currentCustomerClient.value.primaryAddress;
    }
    const addNewAddressExecute = async (addressContact: CompanyProfileAddressContact, addNext: boolean) => {
        ...
        waitModal.value.on = true;
        await (store.dispatch(ActionTypes.CP_UPDATE_ADDRESS_CONTACT, { companyCode: currentCustomerClient.value.companyCode, addressContact: addressContact }))
    ...
    const proceed = async (addNext: boolean) => {
        currentCustomerClient.value.customerCode = customerCode.value;
        emit('proceed', currentCustomerClient.value, addNext);
        resetModal();
    };
    const cancel = async () => {
        emit('close');
    ...
    const resetModal = () => {
        ...
        pic1.value = new CompanyProfileAddressContact();
        pic2.value = new CompanyProfileAddressContact();
        currentCustomerClient.value = new CustomerClient();
    }
</script>
            ...
            <TabView v-model:activeIndex="active">
                <TabPanel :header="t('customer.popups.modifyCustomer.general')">
                    <div class="card mt-3">
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-3 pt-1 text-end no-right-padding">{{t('customer.popups.modifyCustomerClient.clientCode')}}</div>
                                <div class="col-md-3">
                                    <input class="form-control form-control-sm" :maxlength="10" :disabled="props.modalBase.customProps" v-model="currentCustomerClient.code" />
                                </div>
                            </div>
                            <div class="row mt-3">
                                <div class="col-md-3 pt-1 text-end no-right-padding">{{t('customer.popups.modifyCustomerClient.clientName')}}</div>
                                <div class="col-md-9">
                                    <input class="form-control form-control-sm" v-model="currentCustomerClient.name" />
                                </div>
                            </div>
                            <div class="row mt-3">
                                <div class="col-md-3 pt-1 text-end no-right-padding">{{t('companyProfile.companyName')}}</div>
                                <div class="col-md-8">
                                    <select class="form-select form-select-sm" v-model="currentCustomerClient.companyCode" @change="companyChange()">
                ...
                <TabPanel :header="t('customer.popups.modifyCustomer.primaryAddress')">
                    <div class="row mt-3">
                        ...
                        <div class="col-md-8">
                            <select class="form-select form-select-sm" v-model="currentCustomerClient.primaryAddress"
                        ...
                        <div class="col-md-1 no-left-padding text-end">
                            <button class="btn btn-sm btn-primary button-i" :disabled="!currentCustomerClient.companyCode" @click="addNewAddressBook">
                ...
                <TabPanel :header="t('customer.popups.modifyCustomer.billingAddress')">
                    <div class="row mt-3">
                        <div class="col-md-3 pt-1 text-end">{{t('companyProfile.addressBook')}}</div>
                        <div class="col-md-8">
                            <select class="form-select form-select-sm" v-model="currentCustomerClient.billingAddress"
                                    @change="setBillingAddress(currentCustomerClient.billingAddress)">
                                <option v-for="ab in addressBooks.filter((aab)=>aab.status>0)" :key="ab.code" :value="ab.code">{{ab.code}}</option>
                            </select>
                        </div>
                        <div class="col-md-1 no-left-padding text-end">
                ...
                <TabPanel :header="t('customer.popups.modifyCustomer.shippingAddress')">
                    <div class="row mt-3">
                        <div class="col-md-3 pt-1 text-end">{{t('companyProfile.addressBook')}}</div>
                        <div class="col-md-8">
                            <select class="form-select form-select-sm" v-model="currentCustomerClient.shippingAddress"
                                    @change="setShippingAddress(currentCustomerClient.shippingAddress)">
                                <option v-for="ab in addressBooks.filter((aab)=>aab.status>0)" :key="ab.code" :value="ab.code">{{ab.code}}</option>
                            </select>
                        </div>
                        <div class="col-md-1 no-left-padding text-end">
                    ...
                    <div class="card mt-3">
                        ...
                        <div class="card-body">
                            ...
                            <div class="row mt-2">
                                <div class="col-md-3 text-end">{{t('companyProfile.telNo')}}o</div>
                ...
                <TabPanel :header="t('customer.popups.modifyCustomer.pic1')">
                    <div class="row mt-3">
                        ...
                        <div class="col-md-8">
                            <select class="form-select form-select-sm" v-model="currentCustomerClient.pic1" @change="setPic(currentCustomerClient.pic1)">
                                <option v-for="ac in addressContacts.filter((acc)=>acc.status>0)" :key="ac.code" :value="ac.code">{{ac.name}}</option>
                            </select>
                        </div>
                        <div class="col-md-1 no-left-padding text-end">
                            <button class="btn btn-sm btn-primary button-i" :disabled="!currentCustomerClient.primaryAddress" @click="addNewAddressContact">
                                <i class="las la-address-book big-glyphs"></i>
                            </button>
                        </div>
                    </div>
                ...
                <TabPanel :header="t('customer.popups.modifyCustomer.pic2')">
                    <div class="row mt-3">
                        <div class="col-md-3 pt-1 text-end">{{t('customer.popups.modifyCustomer.addressContact')}}</div>
                        <div class="col-md-8">
                            <select class="form-select form-select-sm" v-model="currentCustomerClient.pic2" @change="setPic(currentCustomerClient.pic2, false)">
                                <option v-for="ac in addressContacts.filter((acc)=>acc.status>0)" :key="ac.code" :value="ac.code">{{ac.name}}</option>
                            </select>
                        </div>
                        <div class="col-md-1 no-left-padding text-end">
